...
export default function PersonalPage() {
  ...
  const filteredPersonal = personal.filter(persona => {
    const matchesSearch = `${persona.nombre} ${persona.apellido}`.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const getSituacionBadge = (situacion: string) => {
    switch (situacion) {
      case 'Activo':
        return <Badge variant="default"><CheckCircle className="h-3 w-3 mr-1" />Activo</Badge>;
      case 'Licencia':
        return <Badge variant="secondary"><Clock className="h-3 w-3 mr-1" />En Licencia</Badge>;
      case 'Baja':
        return <Badge variant="destructive"><AlertCircle className="h-3 w-3 mr-1" />Baja</Badge>;
      default:
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Textarea placeholder="Otros títulos obtenidos..." rows={2} />
                    </div>
                    <div>
                      <Label>Especializaciones</Label>
                      <Textarea placeholder="Especializaciones y cursos..." rows={2} />
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Textarea placeholder="Experiencia laboral previa..." rows={3} />
                    </div>
                    <div>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <Dialog>
                      ...
                      <DialogContent className="max-w-2xl">
                        <DialogHeader>
                          <DialogTitle>{persona.nombre} {persona.apellido}</DialogTitle>
                          <DialogDescription>
                        ...
                        <div className="space-y-4">
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <p className="text-sm text-gray-600">CUIL: {persona.cuil}</p>
                              <p className="text-sm text-gray-600">Email: {persona.email}</p>
                              <p className="text-sm text-gray-600">Teléfono: {persona.telefono}</p>
                            </div>
                            <div>
                              <h4 className="font-medium">Datos Laborales</h4>
                              <p className="text-sm text-gray-600">Cargo: {persona.cargo}</p>
                              <p className="text-sm text-gray-600">Nivel: {persona.nivel}</p>
                              <p className="text-sm text-gray-600">Ingreso: {persona.fechaIngreso}</p>
                              <p className="text-sm text-gray-600">Situación: {persona.situacion}</p>
                            </div>
                          </div>
                          
                          <div>
                            <h4 className="font-medium">Formación</h4>
                            <p className="text-sm text-gray-600">{persona.titulo}</p>
                            <p className="text-sm text-gray-600">{persona.institucion}</p>
                          </div>

                          <div>
                            <h4 className="font-medium">Licencias Recientes</h4>
                            <div className="space-y-2">
                              {licencias.filter(l => l.personalId === persona.id).map((licencia) => (
                                <div key={licencia.id} className="flex justify-between text-sm p-2 bg-gray-50 rounded">
                                  <span>{licencia.fechaInicio} - {licencia.tipo}</span>
                                  <Badge variant={licencia.justificada ? 'default' : 'secondary'}>
                                    {licencia.justificada ? 'Justificada' : 'No Justificada'}
                                  </Badge>
                                </div>
                              ))}
                            </div>
              ...
              <Dialog>
                ...
                <DialogContent>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <Select>
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="familiar">Cuidado Familiar</SelectItem>
                    ...
                    <div>
                      <Label>Motivo</Label>
                      <Textarea placeholder="Describa el motivo de la licencia..." rows={3} />
                    </div>
              ...
              <CardContent>
                <div className="space-y-3">
                  {licencias.map((licencia) => {
                    const persona = personal.find(p => p.id === licencia.personalId);
                    return (
                      <div key={licencia.id} className="flex items-center justify-between p-3 border rounded-lg">
                        <div>
                          <p className="font-medium">{persona?.nombre} {persona?.apellido}</p>
                          <p className="text-sm text-gray-600">
                            {licencia.fechaInicio} - {licencia.fechaFin} ({licencia.horas}hs)
                          </p>
                          <p className="text-sm text-gray-600">{licencia.motivo}</p>
                        </div>
